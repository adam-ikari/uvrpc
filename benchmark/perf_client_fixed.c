/**
 * UVRPC Performance Client - Fixed version
...
int main(int argc, char** argv) {
    ...
    while (sent < iterations) {
        int batch_end = (sent + batch_size < iterations) ? sent + batch_size : iterations;
        ...
        /* Run event loop to allow data to be sent and responses to be received */
        for (int i = 0; i < 5; i++) {
    ...
    fprintf(stderr, "Waiting for responses... (%d/%d)\n", g_responses_received, iterations);
    int resp_wait = 0;
    while (!g_done && g_responses_received < iterations && resp_wait < 10000) {
        uv_run(&loop, UV_RUN_ONCE);
        resp_wait++;
        if (resp_wait % 100 == 0) {
            fprintf(stderr, "Progress: %d/%d received\n", g_responses_received, iterations);
        }
    }
