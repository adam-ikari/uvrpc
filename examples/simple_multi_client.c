/**
 * Simple multi-client test for NNG REP server
 */
...
void* client_thread(void* arg) {
    ...
    for (int i = 0; i < 5; i++) {
        nng_msg* msg;
        nng_msg_alloc(&msg, data->payload_size);
        memcpy(nng_msg_body(msg), test_data, data->payload_size);
...
int main(int argc, char** argv) {
    ...
    const char* addr = (argc > 4) ? argv[4] : "tcp://127.0.0.1:5555";
    
    printf("Simple NNG Multi-Client Test\n");
    printf("Clients: %d\n", num_clients);
