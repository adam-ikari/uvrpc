#!/bin/bash
# Async multi-process test

ADDR="${1:-tcp://127.0.0.1:12360}"
NUM_CLIENTS="${2:-10}"
REQUESTS_PER_CLIENT="${3:-100}"
PAYLOAD_SIZE="${4:-128}"

echo "UVRPC Async Multi-Process Test"
echo "================================"
...
# Start server
./build/async_server "$ADDR" > /tmp/async_multi_server.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 0 $((NUM_CLIENTS - 1))); do
    ./build/async_client "$ADDR" "$REQUESTS_PER_CLIENT" "$PAYLOAD_SIZE" > "/tmp/async_client_$i.log" 2>&1 &
    PIDS="$PIDS $!"
...
for i in $(seq 0 $((NUM_CLIENTS - 1))); do
    RECEIVED=$(grep "Received:" "/tmp/async_client_$i.log" 2>/dev/null | awk '{print $2}')
    TIME=$(grep "Total time:" "/tmp/async_client_$i.log" 2>/dev/null | awk '{print $3}')
