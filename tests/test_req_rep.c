...
void* server_thread(void* arg) {
    (void)arg;
    printf("Starting REQ/REP server...\n");
    
    uv_loop_t loop;
    uv_loop_init(&loop);
    
    uvrpc_server_t* server = uvrpc_server_new(&loop, "tcp://127.0.0.1:6003", UVRPC_MODE_REQ_REP);
    if (!server) {
...
/* 客户端测试 */
void client_test() {
    printf("Starting REQ client...\n");
    
    uv_loop_t loop;
    uv_loop_init(&loop);
    
    uvrpc_client_t* client = uvrpc_client_new(&loop, "tcp://127.0.0.1:6003", UVRPC_MODE_REQ_REP);
    if (!client) {
    ...
    /* 发送测试请求 */
    const char* test_data = "Hello, REQ/REP!";
    int completed = 0;
